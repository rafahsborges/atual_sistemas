                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <a class="btn btn-primary btn-spinner btn-sm pull-right m-b-0"
                           href="{{ url('admin/remessas/create') }}" role="button"><i
                                class="fa fa-plus"></i>&nbsp; {{ trans('admin.remessa.actions.create') }}</a>
                    </div>
                        ...
                        <div class="card-block">
                            ...
                            <table class="table table-hover table-listing">
                                ...
                                <tr>
                                    ...
                                    <th is='sortable'
                                        :column="'id_usuario'">{{ trans('admin.remessa.columns.id_usuario') }}</th>
                                    <th is='sortable' :column="'nome'">{{ trans('admin.remessa.columns.nome') }}</th>
                                    <th is='sortable'
                                        :column="'sequencia'">{{ trans('admin.remessa.columns.sequencia') }}</th>
                                    <th is='sortable'
                                        :column="'id_conta'">{{ trans('admin.remessa.columns.id_conta') }}</th>
                                ...
                                <tr v-show="(clickedBulkItemsCount > 0) || isClickedAll">
                                    <td class="bg-bulk-info d-table-cell text-center" colspan="8">
                                            <span class="align-middle font-weight-light text-dark">{{ trans('brackets/admin-ui::admin.listing.selected_items') }} @{{ clickedBulkItemsCount }}.  <a
                                                    ...
                                                    @click="onBulkItemsClickedAll('/admin/remessas')"
                                        ...
                                        <span class="pull-right pr-2">
                                                <button class="btn btn-sm btn-danger pr-3 pl-3"
                                                        @click="bulkDelete('/admin/remessas/bulk-destroy')">{{ trans('brackets/admin-ui::admin.btn.delete') }}</button>
                                            </span>
                                ...
                                <tr v-for="(item, index) in collection" :key="item.id"
                                    ...
                                    <td>@{{ item.id }}</td>
                                    <td>@{{ item.data | datetime('DD/MM/YYYY H:m:s') }}</td>
                            ...
                            <div class="no-items-found" v-if="!collection.length > 0">
                                ...
                                <a class="btn btn-primary btn-spinner" href="{{ url('admin/remessas/create') }}"
                                   role="button"><i
                                        class="fa fa-plus"></i>&nbsp; {{ trans('admin.remessa.actions.create') }}</a>
                            </div>
                        </div>
